...
<?php 
    ...
    GROUP BY tbldistrict.DistrictName where tblgroup.TAID = $ta";
...
<script type="text/javascript">
    ...
    <?php 
        ...
        FROM tblgroup 
        inner join tbldistrict on tblgroup.DistrictID = tbldistrict.DistrictID where ((tblgroup.deleted = '0') and (tblgroup.TAID = $ta))
...
<?php
    ...
    FROM tbljsg inner join tblgroup on tbljsg.groupID = tblgroup.groupID  where tblgroup.TAID = '$ta'";
...
var chart = new google.visualization.ColumnChart(document.getElementById('jsgs_per_district'));
chart.draw(data, options);
}
</script> 
...
<script type="text/javascript">
        ...
        ['District', 'households'],
        <?php 
            $select_query = "SELECT tblbeneficiaries.districtID as District, COUNT(sppCode) as households 
            FROM tblbeneficiaries 
            inner join tblgroup on tblbeneficiaries.groupID = tblgroup.groupID where tblgroup.TAID = '$ta'";
            $query_result = mysqli_query($link,$select_query);
            while($row_val = mysqli_fetch_array($query_result)){
                
            echo "['".dis_name($link,$row_val['District'])."',".$row_val['households']."],";
            }
        ?>
...
<script type="text/javascript">
        ...
        <?php 
            $select_query = "SELECT tblslg_member_savings.districtID as District, SUM(tblslg_member_savings.amount) as TotalSavings
            FROM tblslg_member_savings 
            inner join tblgroup on tblslg_member_savings.groupID = tblgroup.groupID where tblgroup.TAID = '$ta'";
            $query_result = mysqli_query($link,$select_query);
            while($row_val = mysqli_fetch_array($query_result)){
            echo "['".dis_name($link,$row_val['District'])."',".$row_val['TotalSavings']."],";
            }
        ?>
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">                                     
                <div class ="row">
                    <?php
                        if ($_SESSION["user_role"] == '05'){echo '<div class="alert alert-warning" role="alert"> Case Worker Area Summary!</a></div>';}
                    ?>
                    <div class="col-xl-6">
                        <div class="card">
                                        ...
                                        <table class="table table-striped mb-0">
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td>HHs Reached</td>
                                                    <?php
                                                            $result = mysqli_query($link, "SELECT COUNT(sppCode) AS value_sum FROM tblbeneficiaries inner join tblgroup on tblbeneficiaries.groupID = tblgroup.groupID 
                                                ...
                                                <tr>
                                                    ...
                                                    <td>SLGs Formed</td>
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT COUNT(groupID) AS value_sum FROM tblgroup WHERE ((deleted = 0) and (tblgroup.TAID = '$ta'))"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                        $sum = $row['value_sum'];
                                                    ?>
                                                    <td><?php echo "" . number_format($sum);?></td>
                                                    <td><?php echo number_format(0);?></td>
                                                    <td><?php if ($_SESSION["user_role"] == '00'){echo '<a href="javascript: void(0);">more..</a>' ;}else{echo '<a href="basic_livelihood_slg_mgt_filter3_results.php">more ..</a>';}  ?></td>              
                                                </tr>
                                                <tr>
                                                    <th scope="row"><i class='fas fa-book-reader' style='font-size:18px'></i></th>
                                                    <td>SLGs Trained</td>
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT count(distinct(TrainingID)) AS total_grps FROM tblgrouptrainings inner join tblgroup 
                                                        on tblgrouptrainings.groupID = tblgroup.groupID where tblgroup.TAID = '$ta'"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                        $sum = $row['total_grps'];
                                                    ?>
                                                    <td><?php echo "" . number_format($sum);?></td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>HHs Trained</td>
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT sum(Males+Females) AS total_hhs_trained FROM tblgrouptrainings inner join tblgroup 
                                                        on tblgrouptrainings.groupID = tblgroup.groupID where tblgroup.TAID = '$ta'"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                ...
                                                <tr>
                                                    ...
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT COUNT(ClusterID) AS value_cls FROM tblcluster WHERE ((deleted = 0) and (taID = '$ta'))"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                ...
                                                <tr>
                                                    ...
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT COUNT(groupID) AS value_grps FROM tblgroup where ((grad_status='1') and (TAID = '$ta'))"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                        $sum_grps = $row['value_grps'];

                                                        $result_2 = mysqli_query($link, "SELECT COUNT(ClusterID) AS value_clusters FROM tblcluster where ((grad_status='1') and (taID = '$ta'))"); 
                                                ...
                                                <tr>
                                                    ...
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT COUNT(sppCode) AS value_sum FROM tblbeneficiaries inner join tblgroup on tblbeneficiaries.groupID = tblgroup.groupID
                                                        inner join tblta on tblgroup.TAID = tblta.TAID where ((tblta.TAID = '0903') and (tblbeneficiaries.grad_status = '1'))"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                        $sum = $row['value_sum'];
                                                    ?>
                                                    <td><?php echo "" . number_format($sum);?></td>
                                                ...
                                                <tr>
                                                    ...
                                                    <?php
                                                        $result = mysqli_query($link, "SELECT COUNT(groupID) AS v_total FROM tblgroup where ((vc_status = '1') and (TAID = '$ta'))"); 
                                                        $row = mysqli_fetch_assoc($result); 
                                                        $v_total = $row['v_total'];
                                                    ?>
                                                    <td><?php echo "" . number_format($v_total);?></td>
                                                ...
                                                <tr>
                                                    ...
                                                    <?php
                                                        $select_query_esmp = "SELECT COUNT(planID) as TotalESMPs FROM tblsafeguard_group_plans where taID = '$ta'";
                                                ...
                                                <tr>
                                                    ...
                                                    <?php 
                                                        $select_query = "SELECT SUM(amount) as TotalSavings FROM tblslg_member_savings inner join tblgroup on tblslg_member_savings.groupID = tblgroup.groupID
                                                         where (tblgroup.TAID = '$ta')";
                                                        $query_result = mysqli_query($link,$select_query);
