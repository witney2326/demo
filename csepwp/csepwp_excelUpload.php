  ...
  if(in_array($_FILES["file"]["type"],$mimes)){
    ...
    for($i=0;$i<$totalSheet;$i++){
      ...
      foreach ($Reader as $Row)
      {
        ...
        $district = isset($Row[1]) ? $Row[1] : '';
        $household_code = isset($Row[2]) ? $Row[2] : '';
        $household_name = isset($Row[3]) ? $Row[3] : '';
        ...
        $query = "INSERT INTO csepwp_tblhousehold_savings (id,district,household_code,household_name,savings_amount,group_code,date) 
			    VALUES ('".$id."','".$district."', '".$household_code."', '".$household_name."', '".$savings_amount."', '".$group_code."', '".$date."')";
...
?>
<?php
header("Location: http://localhost/cimis/csepwp/csepwp_upload.php");
exit();
?>