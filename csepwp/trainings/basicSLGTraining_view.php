...
$id = $_GET['id']; // get id through query string
        ...
        if (($check == "CLU") or ($check == "CLS"))
        {
                ...
                $resultM = mysqli_query($link_cs, "SELECT SUM(MembersM) AS total_males FROM tblgroup where ClusterID = '$id'"); 
                $rowM = mysqli_fetch_assoc($resultM); 
                $total_males = $rowM['total_males'];

                $resultF = mysqli_query($link_cs, "SELECT SUM(MembersF) AS total_females FROM tblgroup where ClusterID = '$id'"); 
        ...
        function cls_name($link_cs, $clsID)
        {
        $cls_query = mysqli_query($link_cs,"select ClusterName from tblcluster where ClusterID='$clsID'"); // select query
