                ...
                <!-- end page title -->
                            ...
                            <ul class="nav nav-pills nav-justified" role="tablist">
                                <li class="nav-item waves-effect waves-light">
                                    <a class="nav-link "  href="jsg_formation" role="link">
                                        <span class="d-block d-sm-none"><i class="fas fa-home"></i></span>
                                        <span class="d-none d-sm-block">Savings and Loan Groups</span>
                                    </a>
                                </li>
                                <li class="nav-item waves-effect waves-light">
                                    <a class="link"  href="jsg_clusters.php" role="link">
                                ...
                                <li class="nav-item waves-effect waves-light">
                                    <a class="nav-link active" data-bs-toggle="link" href="javasript:void(0)" role="link">
                                        ...
                                        <div class="card-body">
                                            <div class="card border border-primary">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <form class="row row-cols-lg-auto g-3 align-items-center" novalidate action="jsgs_filter3.php" method ="POST">
                                                        <div class="col-12">
                                            ...
                                            <div class="row">
                                                <div class="col-12">
                                                    ...
                                                    <div class="card-header bg-transparent border-primary">
                                                        <h5 class="my-0 text-primary">Joint Skill Groups in <?php echo dis_name($link_cs,$district); ?></h5>
                                                    </div>
                                                    <div class="card-body">
                                                    <h7 class="card-title mt-0"></h7>
                                                            ...
                                                            <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
                                                                ...
                                                                <tbody>
                                                                    <?Php
                                                                        $query="select * from tbljsg where ((districtID = '$district') and (deleted ='0'))";

                                                                        //Variable $link_cs is declared inside config.php file & used here
                                                                        
                                                                        if ($result_set = $link_cs->query($query)) {
                                                                        while($row = $result_set->fetch_array(MYSQLI_ASSOC))
                                                                        { $disname = (string) dis_name($link_cs,$row["districtID"]);
                                                                            $membership = $row["no_male"]+$row["no_female"];
                                                                            $check = substr($row["groupID"], 5, 3);
                                                                
                                                                            if (($check == "CLU") or ($check == "CLS")){
                                                                            $gpID =   $row["groupID"];                                                  
                                                                            $name_query = mysqli_query($link_cs,"select ClusterName from tblcluster where ClusterID='$gpID'"); // select query
                                                                            while($rg = mysqli_fetch_array($name_query)){
                                                                            $clustername = $rg['ClusterName'];}} else{$clustername = "";}
                                                                
                                                                            if ($check == "SLG"){
                                                                            $gpID =   $row["groupID"];
                                                                            $grpname_query = mysqli_query($link_cs,"select groupname from tblgroup where groupID='$gpID'"); // select query
                                                                            while($rg = mysqli_fetch_array($grpname_query)){
                                                                            $groupname = $rg['groupname'];}} else{$groupname ="";}

                                                                            $DbMembers = mysqli_query($link_cs, "SELECT COUNT(sppCode) AS HHs FROM tbljsg_hhs where groupID = '$gpID'"); 
